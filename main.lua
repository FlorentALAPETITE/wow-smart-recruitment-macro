...
function RecrutementEuphorie:PLAYER_ENTERING_WORLD()
    if GetMacroIndexByName(RecrutementEuphorie_MACRO_NAME) == 0 then
        macroId =
            CreateMacro(
            RecrutementEuphorie_MACRO_NAME,
            "ability_warrior_rallyingcry",
            "/run RecrutementEuphorie:MacroClicked();",
            nil
        )
        if macroId == 0 then
    ...
    if RecrutementEuphorie:IsAuthority() then
        ...
        editbox:SetCallback(
            ...
            function(widget, event, text)
                if RecrutementEuphoriePayloadRevision == nil then
                    RecrutementEuphoriePayloadRevision = 0
                end
                RecrutementEuphoriePayloadRevision = GetServerTime()
                RecrutementEuphoriePayloadMessage = text
                RecrutementEuphorie:Print(
                    "Message mis à jour (nouvelle révision : " .. RecrutementEuphoriePayloadRevision .. ")"
                )
...
function RecrutementEuphorie:SendUpdate(notify)
    ...
    RecrutementEuphorie:SendCommMessage(
        RecrutementEuphorie_UPDATE_BROADCAST_PAYLOAD_PREFIX,
        RecrutementEuphorie:SerializePayload(),
        "GUILD"
    )
    if notify then
...
function RecrutementEuphorie:On_REREVISION()
    if RecrutementEuphoriePayloadRevision == nil or RecrutementEuphoriePayloadRevision == 0 then
        RecrutementEuphorie:Print(
            "Aucun message de recrutement enregistré, veuillez patienter qu'un joueur qui le possède se connecte."
        )
    else
...
function RecrutementEuphorie_Load()
    ...
    RecrutementEuphorie:RegisterComm(
        RecrutementEuphorie_REQUEST_BROADCAST_PAYLOAD_PREFIX,
        "On_REQUEST_BROADCAST_PAYLOAD"
    )
